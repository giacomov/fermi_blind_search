...
def rerun_analysis(rerun_analysis_path, met_start, duration, counts, directory, config, logger):

    logger.info("Running an analysis with the following parameters: met_start: %s, duration: %s counts %s "
                "directory: %s" % (met_start, duration, counts, directory))
    ...
    log_path = os.path.join(directory, str(float(met_start)) + "_" + str(float(duration)) + "_farm_log.txt")
    ...
    # rerun_analysis_cmd_line = ("%s --met_start %s --duration %s --counts %s directory %s --config %s" %
    #                            (rerun_analysis_path, met_start, duration, counts, directory,
    #                             config.config_file))
...
def run_real_time_analysis(most_recent_event_time, start_rerun_interval, end_rerun_interval, configuration, logger):
    ...
    with database_connection(configuration):
        ...
        for row in analyses_to_run:
            ...
            rerun_analysis(rerun_analysis_path, row.met_start, row.duration, row.counts, row.directory,
        ...
        if len(most_recent_analysis) == 0:
            ...
            logger.info("The analysis of the most recent data has not been run before, we will run it for the first "
                        "time")
            met_start = most_recent_event_time - end_rerun_interval
            base_path = os.path.abspath(os.path.expandvars(
                os.path.expanduser(configuration.get("Real time", "base_path"))))
            directory = os.path.join(base_path, str(float(met_start)) + "_" + str(float(end_rerun_interval)))
            ...
            rerun_analysis(rerun_analysis_path, most_recent_event_time - end_rerun_interval, end_rerun_interval, 0,
                           directory, configuration, logger)
        else:
