...
def test_add_same_result_twice():
    configuration = read_config_file()
    db = Database(configuration)
    result_to_add = {"met_start": 4.8, "interval": 4.7, "ra": 2.0, "dec": 9.2}
    db.add_candidate(result_to_add)
    with pytest.raises(Exception):
        db.add_candidate(result_to_add)
...
def test_get_results():
    configuration = read_config_file()
    db = Database(configuration)
    result_to_add = {"met_start": 120.9, "interval": 4.7, "ra": 2.0, "dec": 9.2}
    result_to_add2 = {"met_start": 125.1, "interval": 5.2, "ra": 3.0, "dec": 10.2}
    result_to_add3 = {"met_start": 135.1, "interval": 9.2, "ra": 3.0, "dec": 10.2}
    db.add_candidate(result_to_add)
